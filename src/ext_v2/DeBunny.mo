...
actor class EXTNFT(init_owner: Principal) = this {
  ...
  private stable var mintCaller : Principal = Principal.fromText("aaaaa-aa");
  ...
  private stable var config_collection_name : Text  = "DeBunnies";
  private stable var config_collection_data : Text  = "{10,000 Degenerative Bunnies}";
  ...
  public query func ext_mintcaller() : async Principal {
    mintCaller;
  };
  ...
  sc #= "https://"#Principal.toText(a.0)#".raw.icp0.io/: " # debug_show(a.1) # "/" # debug_show(MAX_CHUNK_STORAGE) # "\n";
    };
    return {
      status_code = 200;
      headers = [("content-type", "text/plain")];
  ...
  func _ext_httpAsset(handle : AssetHandle) : HttpResponse {
    switch(_assets.get(handle)){
      case(?asset){        
        switch(asset.atype) {
          case(#direct chunks) _ext_httpOutputAsset(handle, asset, chunks);
          case(#canister a) _ext_httpExternalDisplay(asset, "https://"#a.canister#".raw.icp0.io/?index="#Nat32.toText(a.id));
          case(#other url) _ext_httpExternalDisplay(asset, url);
        };
      };
      case(_) HTTP_NOT_FOUND;
